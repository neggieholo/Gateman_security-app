...
import { getSecurityColleagues } from "../../src/services/api";
...
export default function SecurityColleagues() {
      ...
      <View className="flex-row items-center">
        ...
        <View className="flex-1 ml-4 flex gap-1">
          ...
          <View className="flex-row items-center mt-1">
            ...
            <Text
              className={`${isDarkMode ? "text-slate-300 " : "text-gm-navy"} text-md ml-1 font-oswald-semibold`}
            >
              {item.email}
            </Text>
          </View>
      ...
      {item.is_on_duty && (
        ...
        >
          <View className="flex-1 pr-2">
            <Text
              className={`${isDarkMode ? "text-slate-300 " : "text-gm-navy"} text-[10px] uppercase tracking-widest ml-1 font-oswald-semibold`}
            >
              Check-in Point
            </Text>
            <View className="flex-row items-center mt-1">
              ...
              <Text
                className={`${isDarkMode ? "text-slate-300" : "text-gm-navy"} text-xs ml-1 font-roboto-regular`}
                numberOfLines={1}
              >
  ...
  return (
    <View
      className={`${isDarkMode ? "bg-slate-950" : "bg-gray-50 "} flex-1 p-4`}
    >
      <View className="flex-row justify-between items-center mb-6">
        <View className="flex-1 mr-2">
          <Text
            className={`text-2xl font-montserrat-bold ${isDarkMode ? "text-gray-300" : "text-gray-900"}`}
          >
            Security Team
          </Text>
          <Text className="text-gray-500 font-roboto-regular text-sm">
            {allGuards.length} total guards
          </Text>
        </View>
      </View>
      ...
      <View className="flex-row gap-2 mb-6 justify-evenly">
        {[
          ...
          return (
            ...
            >
              <Text
                className={`font-oswald-semibold text-xs ${isSelected ? (isDarkMode ? "text-gm-gold" : "text-white") : "text-gray-500"}`}
              >
                {item.label}
              </Text>
            </TouchableOpacity>
